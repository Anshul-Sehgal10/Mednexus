...
exports.acceptEmergency = async (req, res) => {
  ...
  try {
    const emergencyRequest = await Emergency.findById(emergencyId).populate(
      "patientId"
    );
    ...
    if (!responder || !emergencyRequest) {
      return res
        .status(400)
        .json({ error: "Invalid doctor or emergency request" });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to accept emergency", details: error.message });
  }
};
...
exports.updateDoctorLocation = async (req, res) => {
  try {
    ...
    if (result.matchedCount === 0) {
      ...
      return res
        .status(404)
        .json({ message: "Emergency not found or not accepted." });
    }
...
exports.getEmergencies = async (req, res) => {
  try {
    const emergencies = await Emergency.find({
      status: { $in: ["pending", "inProgress", "resolved"] },
    }).populate("patientId");
